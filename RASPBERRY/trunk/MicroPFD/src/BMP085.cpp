...
// Calculate true pressure.
	...
	x2 = (-7357 * p) >> 16;
	p += (x1 + x2 + 3791) >> 4;
	baroData.celcius = getCelcius(b5);
	baroData.kPa = (float)p / 1000.0f;
...
int BMP085::getRelativeAltitude () {

    readSensor();
    double n = pow(((double)baroData.kPa  / SEA_LEVEL_PRESSURE), ALT_EXP);
	return (int)(44330.0 * (1.0 - n));
}
...
bool BMP085::getEvent(sensors_vec_t *event) {
    ...
    //event->timestamp = 0;
    event->pressure = baroData.kPa;
    event->temperature = baroData.celcius;
